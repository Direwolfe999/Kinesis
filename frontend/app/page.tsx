...
function DashboardContent() {
    ...
    return (
        <main className="relative min-h-screen bg-[#050505] text-slate-200 overflow-x-hidden font-sans flex">
            {/* Sidebar Navigation */}
            {(activeView !== "warroom" && activeView !== "pipelines") && <Sidebar activeView={activeView} setActiveView={setActiveView} />}

            {/* Floating Action Button */}
            {(activeView !== "warroom" && activeView !== "pipelines") && <FloatingActions addToast={addToast} />}

            {/* Main Content Area - padded left for sidebar */}
            <div className={`flex-1 transition-all duration-300 ${activeView !== 'warroom' && activeView !== 'pipelines' ? 'ml-20' : ''}`}>
                ...
                {activeView === "warroom" || activeView === "pipelines" ? (
                    renderSecondaryView()
                ) : (
